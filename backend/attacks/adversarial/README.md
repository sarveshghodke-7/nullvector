...
$$\{\text{TransactionID}, \text{TransactionAmt}, \text{ProductCD}\}$$

Mutable Set ($X_{\text{mutable}}$): Identity, client, and routing attributes controllable by the client/attacker:

$$\{\text{DeviceType}, \text{DeviceInfo}, \text{Pemaildomain}, \text{Remaildomain}, \text{card4}, \text{card6}\}$$
...
$$f(X_{\text{adv}}) < \tau_{\text{threshold}} \quad \text{where} \quad f(X_{\text{orig}}) \ge \tau_{\text{fraudalert}}$$

(where $f(X)$ is the model's predicted probability of fraud, $\tau_{\text{fraudalert}} = 0.70$, and $\tau_{\text{threshold}} = 0.40$).
